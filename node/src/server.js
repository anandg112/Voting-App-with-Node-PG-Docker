...
app.post('/vote', urlencodedParser, function(req, res){
  ...
  if(vote === 'sandwiches'){
    ...
    client.query('UPDATE votes set number_of_votes=' + votes.sandwiches +
    'where option_name=\'sandwiches\'', function(err, result) {
      if (err) throw err;
    });
    ...
    client.query('UPDATE votes set number_of_votes=' + votes.tacos +
    'where option_name=\'tacos\'', function(err, result) {
      if (err) throw err;
    });
